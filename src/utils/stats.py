...
def calculate_average_response_time(disasters):
    ...
    :param disasters: { (x, y): {"time_step": 2, "end_time_step": 10} }
    :return: float Average response time
    """
    try:
        # 只考虑已经完成救援的灾情点，有time_step和end_time_step的
        response_times = []
        valid_points = 0
        total_points = len(disasters)
        
        for pos, d in disasters.items():
            # 使用时间步而不是时间戳
            if "end_time_step" in d and "time_step" in d:
                response_time = d["end_time_step"] - d["time_step"]
                # 确保响应时间是正数且合理
                if response_time > 0 and response_time < 10000:  # 设置一个合理的上限
