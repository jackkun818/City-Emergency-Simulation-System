...
# 用于动态调整的灾情生成概率
_CUSTOM_DISASTER_SPAWN_RATE = None  # 初始为None表示未使用自定义值
...
SIMULATION_TIME = 300  # 总模拟时间 - 硬编码为300步
...
MARL_CONFIG = {
    ...
    "epsilon_decay": 200,      # 探索率衰减步数
    "replay_buffer_size": 10000, # 经验回放缓冲区大小
    "batch_size": 64,          # 批处理大小
    "target_update": 10,       # 目标网络更新频率
    "model_save_path": "./models/marl_model.pt", # 模型保存路径
}
...
def get_config_param(param_name):
    """根据当前的灾难规模获取对应的配置参数值"""
    # 特殊处理灾情生成概率
    if param_name == "disaster_spawn_rate" and _CUSTOM_DISASTER_SPAWN_RATE is not None:
        return _CUSTOM_DISASTER_SPAWN_RATE
